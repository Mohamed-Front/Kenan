<template>
  ...
  <form @submit.prevent="" class="space-y-10">
    ...
    <div class="card flex justify-content-center">
        ...
        <span class="p-float-label w-full">
            <InputText style="background-color: ##FF7013;" type="password" id="username" class="w-full bg-[#ECF0F1]" v-model="form.password" />
            <label style="font-size: 16px;" for="username">password</label>
        </span>
    </div>
    <div class="flex justify-center mt-4">
      <button style="background-color: #FF7013 !important;border-radius: 10px;color:white" class="w-full lg:w-[50%] py-2 text-lg" @click="authStore.handleLogin(form)">{{ t('auth.login') }}</button>
    </div>
  </form>
</template>
