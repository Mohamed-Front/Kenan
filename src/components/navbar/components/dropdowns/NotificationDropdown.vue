<template>
  <va-dropdown class="notification-dropdown" :offset="[13, 0]" stick-to-edges>
    ...
    <va-dropdown-content class="notification-dropdown__content pl-4 relative pt-[2%]">
      <h3 class="w-full ">Notifications not read</h3>
      <div
        v-for="notification in notificationsProxy"

        class="notification-dropdown__item flex flex-1 flex-wrap p- pb-1 mt-2 mb-2 "

      >
      <p style="border: 1px solid rgb(146, 137, 137);"  class="w-full text-white bg-[#E42222] p-3 rounded-xl ">{{ notification.notification }}</p>
      </div>
